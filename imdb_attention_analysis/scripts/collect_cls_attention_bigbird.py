...
# Create datasets
train_dataset = IMDBDataset(train_texts, train_labels, tokenizer, max_length=1024)
val_dataset = IMDBDataset(val_texts, val_labels, tokenizer, max_length=1024)
test_dataset = IMDBDataset(test_texts, test_labels, tokenizer, max_length=1024)
...
for j, test_data in tqdm(enumerate(test_loader), total=len(test_loader)):
    ...
    concat_max_cls_att = np.vstack([max_cls_attentions, max_cls_joint_attentions])[np.newaxis, :]
    ...
    if (j % 500) == 1:
        print(f"Iteration {j}")
        np.savez_compressed("/u/scratch/z/zhengton/CS263/output/bigbird_concat_max_cls_attentions.npz",
