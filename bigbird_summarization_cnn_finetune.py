...
from transformers import BigBirdTokenizer, BigBirdForTokenClassification  # Changed from BigBird
...
class SummarizationDataset(Dataset):
    def __init__(self, articles, highlights, tokenizer, max_length=4096):  # Changed from 4096
...
def train_model(model, train_loader, val_loader, test_loader, device, criterion, epochs=3):
    ...
    early_stopping = EarlyStopping(patience=3, path='ckpts/best_bigbird_summarization_model.pt')
    ...
    for epoch in range(epochs):
        ...
        checkpoint_path = f'ckpts/bigbird_summarization_epoch_{epoch + 1}.pt'
    ...
    model.load_state_dict(torch.load('ckpts/best_bigbird_summarization_model.pt'))
    return history
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    plt.savefig('ckpts/training_history_bigbird_summarization.png')
    plt.close()
...
def main():
    ...
    tokenizer = BigBirdTokenizer.from_pretrained('google/bigbird-roberta-base')  # Changed from BigBird
    model = BigBirdForTokenClassification.from_pretrained(
        'google/bigbird-roberta-base',  # Changed from BigBird
